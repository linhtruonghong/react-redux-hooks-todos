            ...
            <ul className="list-reset text-black p-0">
                {state.todos.map(todo => (
                    <li key={todo.id}
                        ...
                        <span
                            onDoubleClick={async () => {
                                const response = await axios.patch(`https://hooks-api-kappa.now.sh/todos/${todo.id}`, {
                                    complete: !todo.complete
                                });

                                dispatch({type: "TOGGLE_TODO", payload: response.data})
                            }}
                            className={`flex-1 ml-12 cursor-pointer ${todo.complete && "line-through text-grey"}`}>{todo.text}
                        </span>
                        <button
                        ...
                        <button
                            onClick={async () => {
                                await axios.delete(`https://hooks-api-kappa.now.sh/todos/${todo.id}`);
